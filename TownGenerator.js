...
const kingdomEleArr= [
  'Beliefs', 'Culture', 'Crime', 'Defense', 'Governance', 'Trade'
];
const allKingdomArrays = [
  beliefsArray, cultureArray, crimeArray, defenseArray, govArray, tradeArray
];
/*
...
const fletcherBasic = [
  "Dart +1 (Magical) (Magical)","Shortbow +1 (Magical) (Magical)","Sling +1 (Magical) (Magical)","Blowgun +1 (Magical) (Magical)","Net +1 (Magical) (Magical)"
];
const fletcherBetter = [
  "Crossbow, light +1 (Magical)","Dart +2 (Magical)","Shortbow +2 (Magical)","Sling +2 (Magical)","Blowgun +1 (Magical)","Crossbow, hand +1 (Magical)","Crossbow, heavy +1 (Magical)","Longbow +1 (Magical)","Net +2 (Magical)", "Ammunition, +2 (Magical)", "Ammunition, +1 (Magical)"
];
const fletcherBest = [
  "Crossbow, light +2 (Magical)","Dart +3 (Magical)","Shortbow +3 (Magical)","Sling +3 (Magical)","Blowgun +2 (Magical)","Crossbow, hand +2 (Magical)","Crossbow, heavy +2 (Magical)","Longbow +2 (Magical)","Net +3 (Magical)", "Ammunition, +3 (Magical)", "Ammunition, +2 (Magical)"
];
const healerBasic = [
...
const priests = [];
const weaponsBasic = [
"Club +1 (Magical)","Dagger +1 (Magical)","Greatclub +1 (Magical)","Handaxe +1 (Magical)","Javelin +1 (Magical)","Lig⁠ht ham⁠mer +1 (Magical)","Mace +1 (Magical)","Sickle +1 (Magical)","Spear +1 (Magical)","Battleaxe +1 (Magical)","Flail +1 (Magical)","Glaive +1 (Magical)","Greataxe +1 (Magical)","Halberd +1 (Magical)","Lance +1 (Magical)","Maul +1 (Magical)","Morningstar +1 (Magical)","Pike +1 (Magical)","Trident +1 (Magical)","War pick +1 (Magical)","Warhammer +1 (Magical)","Whip +1 (Magical)"
];
const weaponsBetter = [
  "Club +2 (Magical)","Dagger +2 (Magical)","Greatclub +2 (Magical)","Handaxe +2 (Magical)","Javelin +2 (Magical)","Lig⁠ht ham⁠mer +2 (Magical)","Mace +2 (Magical)","Quarterstaff +1 (Magical)","Sickle +2 (Magical)","Spear +2 (Magical)","Battleaxe +2 (Magical)","Flail +2 (Magical)","Glaive +2 (Magical)","Greataxe +2 (Magical)","Halberd +2 (Magical)","Lance +2 (Magical)","Longsword +1 (Magical)","Maul +2 (Magical)","Morningstar +2 (Magical)","Pike +2 (Magical)","Rapier +1 (Magical)","Scimitar +1 (Magical)","Shortsword +1 (Magical)","Trident +2 (Magical)","War pick +2 (Magical)","Warhammer +2 (Magical)","Whip +2 (Magical)","Silver Components","Silver Components","Silver Components", "Weapon of Warning (Roll)", "Weapon of Warning (Roll)", "Weapon of Warning (Roll)"
];
const weaponsBest = [
  "Greatsword +1 (Magical)", "Greatsword +2 (Magical)","Club +3 (Magical)","Dagger +3 (Magical)","Greatclub +3 (Magical)","Handaxe +3 (Magical)","Javelin +3 (Magical)","Lig⁠ht ham⁠mer +3 (Magical)","Mace +3 (Magical)","Quarterstaff +2 (Magical)","Sickle +3 (Magical)","Spear +3 (Magical)","Battleaxe +3 (Magical)","Flail +3 (Magical)","Glaive +3 (Magical)","Greataxe +3 (Magical)","Halberd +3 (Magical)","Lance +3 (Magical)","Longsword +2 (Magical)","Maul +3 (Magical)","Morningstar +3 (Magical)","Pike +3 (Magical)","Rapier +2 (Magical)","Scimitar +2 (Magical)","Shortsword +2 (Magical)","Trident +3 (Magical)","War pick +3 (Magical)","Warhammer +3 (Magical)","Whip +3 (Magical)","Silver Components","Silver Components","Silver Components","Silver Components","Silver Components","Silver Components","Silver Components", "Weapon of Warning (Roll)", "Weapon of Warning (Roll)", "Weapon of Warning (Roll)", "Weapon of Warning (Roll)"
];
...
const AddItem = (arr, customizationArr, desc) => {
  ...
  if(desc == 'Beliefs') {
    ...
    temp = `<i><b>${desc}</i></b> - ${temp}`;
    kingdom.push(temp);
    return kingdom;
  };
const AddKingdomTable = () => {
  kingdom = [];
  let table = document.createElement('table');
  table.classList.add('zui-table');
  let tHeader = document.createElement('thead');
  tHeader.innerHTML = `<th>Element</th><th>Priority</th>`;
  table.appendChild(tHeader);
  let tBody = document.createElement('tbody');
  for (let i = 0, j = kingdomEleArr.length; i < j; i++) {
    AddItem(kingdom, allKingdomArrays[i], kingdomEleArr[i]);
  }
  impact.sort(Func);
  kingdom.sort(Func);
  for (let i = 0, j = kingdom.length; i < j; i++){
    WriteRow([kingdom[i], impact[i]], true);
    tBody.appendChild(row);
  }
  table.appendChild(tBody);
   AddDivUsingArray('kingdom', table, "Area Elements");
};
...
/*
  HTML table building
*/
const WriteRow = (args, html) => {
  row = document.createElement('tr');
  for (x = 0, y = args.length; x < y; x++) {
    let cell = document.createElement('td');
    if (html === true) {
      cell.innerHTML = (args[x]);
    } else {
...
const WriteBody = (arr) => {
  ...
  for (let x = 0, y = arr.length; x < y; x++) {
    ...
    c = arr[x].direction;
    b = arr[x].quality;
    a = `${arr[x].type} - ${arr[x].description}`;
...
const AddDivUsingArray = (id, output, title, casterLvl) => {
  ...
  if (casterLvl) {
    divHeader.innerHTML = title+'<i class="caster"> / Level '+casterLvl+'</i>'
  } else {
...
const BuildAllObjList = (arr) => {
  ...
  for (let t = 0, u = poiArray.length; t < u; t++) {
    ...
    if(filteredArr.length) {
      ...
      let tHeader = document.createElement('thead');
      let tBody = document.createElement('tbody');
      let headerInputs = ['Description','Quality','District'];
      for (let x = 0; x < 3; x++) {
...
const Execute = () => {
  ...
  id = 1;
  dID = 1;
  AddSectionHTML('kingdom', 'Area Elements')
