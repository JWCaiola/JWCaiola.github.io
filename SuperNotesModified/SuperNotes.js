...
on('ready', () => {
    ...
    on('chat:message', function(msg) {
        if ('api' === msg.type && msg.content.match(/^!(gm|pc)note\b/)) {
            ...
            let whisperTo = msg.content.split(/\s+--/)[2];
            ...
            if (option !== undefined && option.includes('config')) {
                ...
                if (option !== undefined && option.includes('whisper')) {
                  if (whisperTo === undefined) return;
                  messagePrefix = `/w ${whisperTo} `;
                }
                if (option !== undefined && option.includes('help')) {
                    ...
                    if (!(option + '').match(/^(bio|whisper|charnote|avatar|imag(e|es|e[1-9]))/)) {
                        option = 'token';
                    }
